...
private DogAction checkConstructors(String[] tokens)
{
  if (tokens.length >= 5)
  {
      ...
      try {
        ...
        System.err.println("CodeCompiler Error: could not process " + Arrays.toString(tokens) + "\n");
          ...
          for (Method method : Method.values())
          {
            if (methodIdentifier.equals(method.getMethodIdentifier()))
            {
              ...
              if (tokens[tokens.length-1].substring(tokens[tokens.length-1].length()-2).equals(");"))
              {
                ...
                // Match number of arguments provided with number of parameters defined. No overloading.
                if (arguments.length ==  method.getNumberOfArguments())
                {
                  dogActionBuilder.setArgs(arguments);
                }
